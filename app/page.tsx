    ...
    <main className="flex-1 flex flex-col">
      <section className="mx-auto w-full max-w-[900px] flex flex-col items-center text-center gap-6 px-4 md:px-8 py-16 md:py-24">
        <h1 className="text-4xl md:text-6xl font-extrabold leading-tight">
          ...
          <span className="block text-base font-normal text-muted mt-2">
        ...
        <Link
          href="/test"
          className="inline-block px-8 py-4 rounded-md bg-primary text-white text-lg font-semibold shadow-elevated hover:bg-primary/90"
        >
          Пройти тест
        </Link>
        <p className="text-sm text-muted">5–7 минут, 8 раундов, без регистрации</p>
      </section>
        ...
        <div className="mx-auto w-full max-w-[1200px] px-4 md:px-8 py-12 md:py-16 flex flex-col gap-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {MODALITIES.map((m) => (
              <div
                key={m.name}
                className="flex flex-col items-center text-center gap-3 p-5 bg-bg rounded-md border border-border"
              >
